...
export function assertOfficialOpenMeteoUrl(value: URL | string): URL {
  ...
  if (
    ...
    url.password ||
    url.port ||
    !OPEN_METEO_HOSTS.has(url.hostname.toLowerCase())
  ) {
    throw new Error('Solicitud Open-Meteo rechazada: endpoint no oficial');
  }
  return url;
}

function stripOpenMeteoApiKeyParams(url: URL): void {
  for (const name of [...url.searchParams.keys()]) {
    if (name.toLowerCase() === 'apikey') url.searchParams.delete(name);
  }
}

export function openMeteoCacheKey(value: URL | string): string {
  const url = assertOfficialOpenMeteoUrl(value);
  stripOpenMeteoApiKeyParams(url);
