...
describe('OpenMeteoClientService', () => {
  ...
  it('rechaza HTTPS con hostname falsificado antes de cachear o enviar', () => {
    const spoofed =
      'https://customer-api.open-meteo.com.attacker.example/v1/forecast';
    expect(() => assertOfficialOpenMeteoUrl(spoofed)).toThrow(
      /endpoint no oficial/,
    );
    expect(() => openMeteoCacheKey(spoofed)).toThrow(/endpoint no oficial/);
    expect(() =>
      withOpenMeteoApiKey(spoofed, 'secret-that-must-not-leak'),
    ).toThrow(/endpoint no oficial/);
  });
  ...
  it('elimina apikey del cache y request sin distinguir mayusculas', () => {
    const leaked = 'uppercase-query-secret';
    const source = `https://customer-api.open-meteo.com/v1/forecast?APIKEY=${leaked}&latitude=-33`;

    expect(openMeteoCacheKey(source)).not.toContain(leaked);
    const request = withOpenMeteoApiKey(source, 'replacement-key');
    expect(request).not.toContain(leaked);
    expect(new URL(request).searchParams.get('apikey')).toBe('replacement-key');
  });
