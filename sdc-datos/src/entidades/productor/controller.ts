import {
  Controller,
  Get,
  Post,
  Body,
  Param,
  Delete,
  Query,
  Put,
} from '@nestjs/common';
import { ProductorsService } from './service';
import { ICreateProductor, IQueryParam, ISolicitudArchivado, IUpdateProductor } from 'modelos/src';
import { ApiTags } from '@nestjs/swagger';

@ApiTags('Productors')
@Controller('productors')
export class ProductorsController {
  constructor(private readonly service: ProductorsService) {}

  @Get()
  async getFilter(@Query() query: IQueryParam) {
    return await this.service.getFilter(query);
  }

  @Get(':id')
  async getById(@Param('id') id: string) {
    return await this.service.getById(id);
  }

  @Post()
  async create(@Body() data: ICreateProductor) {
    return await this.service.create(data);
  }

  @Put(':id')
  async update(@Param('id') id: string, @Body() data: IUpdateProductor) {
    return await this.service.update(id, data);
  }

  @Delete(':id')
  async delete(@Param('id') id: string, @Query() audit: ISolicitudArchivado) {
    return await this.service.delete(id, audit);
  }
}
