...
export interface OpenMeteoGetJsonOptions {
  /**
   * La cache vencida solo sirve para continuidad visual. Los motores que
   * abren o cierran alertas deben pedir explicitamente datos frescos.
   */
  allowStale?: boolean;
}
...
export class OpenMeteoClientService {
  ...
  private readonly cacheMs = 20 * 60 * 1000;
  ...
  async getJson<T = any>(
    url: URL,
    context: string,
    options: OpenMeteoGetJsonOptions = {},
  ): Promise<T | null> {
    ...
    if (this.providerBlockedUntil > now) {
      return allowStale
        ? this.stale<T>(safeUrl, cached, context, 'proveedor en espera')
        : null;
    }

    const inFlight = this.pending.get(safeUrl);
    if (inFlight) {
      const data = (await inFlight) as T | null;
      return (
        data ??
        (allowStale
    ...
    if (this.circuitOpenUntil > now) {
      return allowStale
        ? this.stale<T>(safeUrl, cached, context, 'circuito abierto')
        : null;
    }
    if (this.pending.size >= this.maxPendingRequests) {
      this.logger.warn(
        `Open-Meteo ${sanitizeOpenMeteoMessage(context, OPEN_METEO_API_KEY)}: cola completa; no se agrega otra solicitud.`,
      );
      return allowStale
        ? this.stale<T>(safeUrl, cached, context, 'cola completa')
        : null;
    }

    // `pending` representa solo la consulta fresca. Asi dos consumidores con
    // politicas distintas pueden compartir la salida al proveedor sin que un
    // dato stale habilitado para UI se filtre a un motor de decisiones.
    const deadlineAt = now + this.requestDeadlineMs;
    const request = this.fetchFresh<T>(safeUrl, context, deadlineAt).finally(
      () => this.pending.delete(safeUrl),
    );
  ...
  private async fetchFresh<T>(
    ...
    const safeContext = sanitizeOpenMeteoMessage(context, OPEN_METEO_API_KEY);
    for (let attempt = 0; attempt <= OPEN_METEO_MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        try {
          ...
          try {
            response = await fetch(
              withOpenMeteoApiKey(safeUrl, OPEN_METEO_API_KEY),
              {
            ...
            if (response.ok) {
              ...
              if (sizeBytes <= this.maxCacheEntryBytes) {
                ...
                this.cache.set(safeUrl, {
                  expiresAt: savedAt + this.cacheMs,
                  staleUntil: savedAt + this.cacheMs + this.staleMs,
                  data,
                  sizeBytes,
                });
                this.cacheBytes += sizeBytes;
                this.pruneCache(savedAt);
              } else {
                this.logger.warn(
        ...
        if (attempt < OPEN_METEO_MAX_RETRIES) {
          this.logger.warn(
            `Error Open-Meteo ${safeContext}; reintento en ${waitMs} ms: ${sanitizeOpenMeteoMessage(error, OPEN_METEO_API_KEY)}`,
          );
          continue;
        }
        this.logger.error(
          `Error al obtener Open-Meteo ${safeContext}: ${sanitizeOpenMeteoMessage(error, OPEN_METEO_API_KEY)}`,
        );
        this.registerFailure();
        return null;
      }
  ...
  private stale<T>(
    ...
    this.logger.warn(
      `Open-Meteo ${sanitizeOpenMeteoMessage(context, OPEN_METEO_API_KEY)}: usando cache de emergencia (${reason}).`,
    );
    return cached.data as T;
  }
