...
$migrationId = 'production-barley-pea-current-20260718'
$databaseName = 'chaman'
$snapshotScript = 'scripts/migrations/20260718-production-barley-pea-snapshot.js'
...
if (-not $Apply) {
  & railway run --service MongoDB --environment production node $snapshotScript plan
  exit $LASTEXITCODE
}
...
$planRaw = & railway run --service MongoDB --environment production node $snapshotScript plan
if ($LASTEXITCODE -ne 0) { throw 'No se pudo obtener el plan productivo.' }
$plan = (($planRaw -join "`n") | ConvertFrom-Json).result
$sowingIds = @($plan.latest.rows | Where-Object status -ne 'actual' | ForEach-Object idSiembra)
if ($plan.targetSowings -eq 0) { throw 'Plan inconsistente: no se iniciara la reconstruccion.' }
if ($sowingIds.Count -eq 0) {
  [pscustomobject]@{ database = $databaseName; backupId = $BackupId; requested = 0; rebuilt = 0; status = 'already-current' } |
    ConvertTo-Json
  exit 0
}
...
$dataLog = Join-Path $logDir 'production-barley-pea-data.log'
$dataErrorLog = Join-Path $logDir 'production-barley-pea-data.error.log'
$climateLog = Join-Path $logDir 'production-barley-pea-climate.log'
$climateErrorLog = Join-Path $logDir 'production-barley-pea-climate.error.log'
$predictionLog = Join-Path $logDir 'production-barley-pea-predictions.log'
$predictionErrorLog = Join-Path $logDir 'production-barley-pea-predictions.error.log'
...
try {
  ...
  foreach ($id in $sowingIds) {
    ...
    try {
      ...
      throw "Fallo la reconstruccion de $id; el respaldo permite rollback exacto."
    }
  }
